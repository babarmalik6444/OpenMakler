<?php

namespace App\Policies\Openimmo;

use App\Models\Openimmo\RealEstate;
use App\Models\User;
use Illuminate\Auth\Access\HandlesAuthorization;

class RealEstatePolicy
{
    use HandlesAuthorization;

    /**
     * Determine whether the user can view any models.
     *
     * @param \App\Models\User $user
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function viewAny(User $user)
    {
        return true;
    }


    /**
     * Determine whether the user can view the model.
     *
     * @param \App\Models\User $user
     * @param \App\Models\Openimmo\RealEstate $realEstate
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function view(User $user, RealEstate $realEstate)
    {
        return !$realEstate->exists ||
            $user->isSystemAdminOrSystemUser() ||
            $user->company_id == $realEstate->company_id;
    }


    /**
     * Determine whether the user can create models.
     *
     * @param \App\Models\User $user
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function create(User $user)
    {
        return $user->company_id;
    }


    /**
     * Determine whether the user can update the model.
     *
     * @param \App\Models\User $user
     * @param \App\Models\Openimmo\RealEstate $realEstate
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function update(User $user, RealEstate $realEstate)
    {
        return $user->isSystemAdminOrSystemUser() ||
            ($user->isOwner() && $realEstate->company_id == $user->company_id) ||
            ($user->isUser() && $user->company_office_id == $realEstate->company_office_id);
    }


    /**
     * Determine whether the user can delete the model.
     *
     * @param \App\Models\User $user
     * @param \App\Models\Openimmo\RealEstate $realEstate
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function delete(User $user, RealEstate $realEstate)
    {
        return $user->isSystemAdminOrSystemUser() ||
            ($user->isOwner() && $realEstate->company_id == $user->company_id) ||
            ($user->isUser() && $user->company_office_id == $realEstate->company_office_id);
    }


    /**
     * Determine whether the user can restore the model.
     *
     * @param \App\Models\User $user
     * @param \App\Models\Openimmo\RealEstate $realEstate
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function restore(User $user, RealEstate $realEstate)
    {
        return false;
    }


    /**
     * Determine whether the user can permanently delete the model.
     *
     * @param \App\Models\User $user
     * @param \App\Models\Openimmo\RealEstate $realEstate
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function forceDelete(User $user, RealEstate $realEstate)
    {
        return false;
    }
}
