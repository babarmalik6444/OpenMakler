...
class AuthService
{
  ...
  protected string $scout24_token_url;
    //public string $return_url = 'http://openmakler2';
  ...
  { 
    try
    {
      ...
      $middleware = new Oauth1(['consumer_key' => $this->oauth1_key,
        ...
        'callback' => $this->oauth1_callbackUrl.'/app/real-estates/' . $reast_estate_id . '/edit'
      ]);
      $stack->push($middleware);

      $client = new Client(['base_uri' => $this->is24_domain, 'handler' => $stack, 'verify' => false]);
  ...
  public function getAccessToken($token, $verifier, $id)
  {
    try
    {
      ...
      $ScoutData = ScouteApi::latest()->first();
            //dd($ScoutData);
      $middleware = new Oauth1(['consumer_key' => $this->oauth1_key, 'consumer_secret' => $this->oauth1_secret, 'callback' => $this->oauth1_callbackUrl.'/app/real-estates/' . $id . '/edit', 'token_secret' => $ScoutData->oauth_token_secret, 'verifier' => $verifier, 'token' => $ScoutData->oauth_token, 'request_method' => Oauth1::REQUEST_METHOD_HEADER, 'signature_method' => Oauth1::SIGNATURE_METHOD_HMAC]);
  ...
  public function addProperty($record)
  {
    ...
    try
    {
      ...
      $middleware = new Oauth1(['consumer_key' => $this->oauth1_key, 'consumer_secret' => $this->oauth1_secret, 'callback' => $this->oauth1_callbackUrl.'/app/real-estates/' . $record->id . '/edit', 'token_secret' => $ScoutData->oauth_token_secret, 'verifier' => $ScoutData->verifier, 'token' => $ScoutData->oauth_token, 'request_method' => Oauth1::REQUEST_METHOD_HEADER, 'signature_method' => Oauth1::SIGNATURE_METHOD_HMAC]);

      $stack->push($middleware);

      $client = new Client(['base_uri' => $this->is24_domain, 'handler' => $stack, 'verify' => false, 'headers' => $headers]);
            // Set the "auth" request option to "oauth" to sign using oauth
